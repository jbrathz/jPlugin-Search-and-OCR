...
class PDFS_Queue_Service {
    ...
    public static function process_batch_realtime($batch_id) {
        ...
        foreach ($file_ids as $file_id) {
            // Wrap each file processing in try-catch to prevent batch crash
            try {
                ...
                // Catch any unexpected errors to prevent batch crash
                $error_count++;
                $results[] = array(
                    'file_id' => $file_id,
                    'status' => 'error',
                ...
                PDFS_Logger::error('Batch processing crashed', array(
                    'batch_id' => $batch_id,
                    'file_id' => $file_id,
                    'error' => $e->getMessage(),
    ...
    public static function process_media_batch_realtime($batch_id) {
        ...
        foreach ($attachment_ids as $attachment_id) {
            ...
            try {
                ...
                PDFS_Logger::error('Media batch processing crashed', array(
                    'batch_id' => $batch_id,
                    'attachment_id' => $attachment_id,
                    'error' => $e->getMessage(),
                    'trace' => $e->getTraceAsString(),
